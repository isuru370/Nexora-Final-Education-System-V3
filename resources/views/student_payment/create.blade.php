...
@push('scripts')
    ...
    <script>
        class PaymentSystem {
            ...
            setupEventListeners() {
                ...
                if (searchBtn) {
                    searchBtn.addEventListener('click', () => this.searchStudent());
                }
                
                const studentIdInput = document.getElementById('studentCustomId');
                if (studentIdInput) {
                ...
                if (monthSelect) monthSelect.addEventListener('change', () => this.loadAttendanceData());
                if (yearSelect) yearSelect.addEventListener('change', () => this.loadAttendanceData());
                
                const useDefaultBtn = document.getElementById('useDefaultFee');
                if (useDefaultBtn) useDefaultBtn.addEventListener('click', () => this.useDefaultFee());
                
                const processBtn = document.getElementById('processPaymentBtn');
                if (processBtn) processBtn.addEventListener('click', () => this.submitPayment());
            }
            ...
            setCurrentDate() {
                ...
                if (paymentDate) paymentDate.value = currentDate.toISOString().split('T')[0];
            }
            ...
            async toggleQRScanner() {
                ...
                if (this.qrScanner) {
                    this.stopQRScanner();
                    if (scannerBtn) scannerBtn.innerHTML = '<i class="fas fa-camera me-2"></i>Start QR Scanner';
                    return;
                }

                try {
                    if (scannerBtn) scannerBtn.innerHTML = '<i class="fas fa-spinner fa-spin me-2"></i>Loading Scanner...';
                    ...
                    if (scannerBtn) scannerBtn.innerHTML = '<i class="fas fa-stop me-2"></i>Stop Scanner';
            ...
            onQRCodeScanned(decodedText) {
                ...
                const studentIdInput = document.getElementById('studentCustomId');
                if (studentIdInput && customId) {
                    studentIdInput.value = customId;
            ...
            async searchStudent(customId = null) {
                ...
                try {
                    // First API call: Get student by QR code
                    const studentResponse = await fetch(`/api/read-qr-code/student-id?qr_code=${encodeURIComponent(searchId)}`);
                    ...
                    // Second API call: Get student classes with filter (matches your backend)
                    const classesResponse = await fetch(`/api/student-classes/student/${this.studentId}/filter`);
                    
                    if (!classesResponse.ok) throw new Error('Unable to fetch student classes');
                    const classesData = await classesResponse.json();
                    
                    if (classesData.status === 'success' && classesData.data && classesData.data.length > 0) {
                        this.currentClassesData = classesData.data;
                        this.currentStudentData = classesData.data[0];
                        this.displayStudentInfo(this.currentStudentData);
                        this.displayClasses(classesData.data);
            ...
            displayStudentInfo(studentData) {
                ...
                if (studentDetails && studentInfoCard) {
                    studentDetails.innerHTML = `
                        ...
                        <div class="col-md-5">
                            ...
                            <p class="mb-1"><strong>Grade:</strong> ${studentData.student_class?.grade?.grade_name || 'N/A'}</p>
                        </div>
                        <div class="col-md-5">
            ...
            displayClasses(classesData) {
                ...
                classesData.forEach((classData, index) => {
                    ...
                    const classFee = classData.class_category_has_student_class?.class_fee || classData.default_fee || 0;
                    const categoryName = classData.class_category?.category_name || 'N/A';

                    const colors = ['primary', 'success', 'info', 'warning', 'danger', 'secondary'];
                    const colorIndex = (classData.student_class?.class_name?.length || index) % colors.length;
                                ...
                                <div class="card-header bg-gradient bg-${classInactive ? 'secondary' : themeColor} text-white py-3 border-0">
                                    <div class="d-flex justify-content-between align-items-start">
                                        <div class="flex-grow-1">
                                            <h6 class="card-title mb-1 fw-bold text-truncate">${classData.student_class?.class_name || 'N/A'}</h6>
                                            <div class="d-flex gap-2 align-items-center">
                                                <span class="badge bg-white bg-opacity-20 text-dark border-0">${categoryName}</span>
                                ...
                                <div class="card-body">
                                    <div class="position-relative mb-4">
                                        <span class="badge bg-${classInactive ? 'secondary' : themeColor} px-3 py-2">
                                            <i class="fas fa-layer-group me-1"></i>${classData.student_class?.grade?.grade_name || 'N/A'}
                                        </span>
                                    </div>
                                    ...
                                    <div class="class-details mb-3">
                                        <div class="detail-item d-flex align-items-center mb-2">
                                            ...
                                            <div>
                                                ...
                                                <span class="fw-semibold">${classData.student_class?.subject?.subject_name || 'N/A'}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div class="detail-item d-flex align-items-center mb-2">
                                            ...
                                            <div>
                                                ...
                                                <span class="fw-semibold">${classData.student_class?.teacher?.first_name || ''} ${classData.student_class?.teacher?.last_name || 'N/A'}</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="fee-section bg-light bg-opacity-50 rounded p-3 mb-3">
                                        ...
                                        ${classData.custom_fee && classData.custom_fee != classData.default_fee ? `
                                        <div class="d-flex justify-content-between align-items-center mt-1">
                                            <span class="text-muted small">Original Fee:</span>
                                            <span class="text-muted small">LKR ${this.formatCurrency(classData.default_fee)}</span>
                                        </div>
                                        ` : ''}
                                    </div>
                                        ...
                                        ` : `
                                            <div class="d-grid gap-2">
                                                <button class="btn btn-success btn-pay shadow-sm py-2" 
                                                        ...
                                                        data-fee="${classFee}"
                                                        data-class-name="${classData.student_class?.class_name || 'N/A'}"
                                                        data-grade="${classData.student_class?.grade?.grade_name || 'N/A'}"
                ...
                if (classesCard) classesCard.style.display = 'block';
            ...
            openPaymentModal(studentClassId, fee, className, grade, guardianMobile) {
                ...
                const studentClassIdInput = document.getElementById('payment_student_class_id');
                const guardianMobileInput = document.getElementById('payment_guardian_mobile');
                const amountInput = document.getElementById('payment_amount');
                const defaultFeeSpan = document.getElementById('defaultFee');
                
                if (studentIdInput) studentIdInput.value = this.studentId;
                if (studentClassIdInput) studentClassIdInput.value = studentClassId;
                if (guardianMobileInput) guardianMobileInput.value = guardianMobile;
                if (amountInput) amountInput.value = fee;
                if (defaultFeeSpan) defaultFeeSpan.textContent = this.formatCurrency(fee);
                ...
                if (modalStudentName) modalStudentName.textContent = this.currentStudentData?.student?.last_name || this.currentStudentData?.student?.first_name || '-';
                if (modalStudentId) modalStudentId.textContent = this.currentStudentData?.student?.student_custom_id || '-';
    ...
    const noAttendanceInfo = document.getElementById('noAttendanceInfo');
    
    if (attendanceLoading) attendanceLoading.style.display = 'block';
    if (attendanceInfo) attendanceInfo.style.display = 'none';
    if (noAttendanceInfo) noAttendanceInfo.style.display = 'none';

    // ✅ නිවැරදි URL එක - month එක string විදියට send කරන්න
    const url = `/api/attendances/monthly/${studentId}/${studentClassId}/${year}-${month}`;
    
    console.log('Fetching attendance from:', url); // Debug සඳහා
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            if (data.status === 'success' && data.data) {
                ...
                document.getElementById('attendanceCount').textContent = data.data.attendance_count || 0;
                document.getElementById('weeksInMonth').textContent = data.data.weeks_in_month || 0;
            } else {
                if (noAttendanceInfo) noAttendanceInfo.style.display = 'block';
            }
        })
        .catch(error => {
            ...
            useDefaultFee() {
                const defaultFeeSpan = document.getElementById('defaultFee');
                const amountInput = document.getElementById('payment_amount');
                if (defaultFeeSpan && amountInput) {
                    const defaultFee = parseFloat(defaultFeeSpan.textContent.replace(/,/g, '')) || 0;
    ...
    try {
        ...
        const studentId = parseInt(document.getElementById('payment_student_id')?.value || 0);
        const studentClassId = parseInt(document.getElementById('payment_student_class_id')?.value || 0);
        ...
        if (data.status === 'success') {
            ...
            setTimeout(() => {
                const studentIdInput = document.getElementById('studentCustomId');
                if (studentIdInput && this.currentStudentData?.student?.student_custom_id) {
        ...
        btn.innerHTML = originalText;
            ...
            showAlert(message, type) {
                ...
                if (!alertContainer) {
                    ...
                    if (container) container.prepend(alertContainer);
                }
